...
nbe
ich habe jetzt schon mal ganz viel auskommentiert, damit ich mich langsam dahinarbeite, eine einfache Uhr zu erstellen, die ich per knopfdruck ein- und wieder ausschalten kann.
mehr will ich nicht, das bekomme ich aber evtl. nur über diesen weg. 
*/
...
LV_FONT_DECLARE(seven_segment_56);
LV_FONT_DECLARE(seven_segment_40);
LV_IMG_DECLARE(bg);
LV_IMG_DECLARE(bg1);
LV_IMG_DECLARE(bg2);
LV_IMG_DECLARE(bg3);
...
// hier mal die nicht funktionierenden Einträge auskommentiert

MenuBar::lv_menu_config_t _cfg[1] = {
...
{
    /*
     * erstmal wird nur die Uhrzeit angezeigt, ansonsten nix. da brauchen wir keinen event handler
     * 
...
void setupGui()
{
    ...
    timeStyle.text.font = &seven_segment_40; //Ubuntu; // seconds do not fit with the font ubuntu, 
    //timeStyle.text.font = &Geometr; // was just a test, but too small 
...
static void updateTime()
{
    ...
    strftime(buf, sizeof(buf), "%H:%M:%S", &info); // skip the colon, then we can display seconds as well
    lv_label_set_text(timeLabel, buf);
    // lv_obj_align(timeLabel, NULL, LV_ALIGN_IN_TOP_MID, 0, 20);
    lv_obj_align(timeLabel, NULL, LV_ALIGN_CENTER, 0, -10);
}
